...
public:
   FileTracer (void) {  
     addTracePattern  (L"Tales of Berseria");
     addIgnorePattern (L"\\\\?\\");
...
ReadFile_Detour ( _In_      HANDLE       hFile,
                ...
                _Inout_opt_ LPOVERLAPPED lpOverlapped )
{
  if (tbf::tracer->isTracedFile (hFile)) {
                     ...
                     _In_opt_ HANDLE                hTemplateFile )
{
  ...
  if (hFile != 0) {
    ...
    if (wcslen (wszFileName))
      tbf::tracer->addFile (wszFileName, hFile);
  }
...
void
tbf::FileIO::Init (void)
{
  TBF_CreateDLLHook2 ( L"kernel32.dll", "CreateFileA",
                       CreateFileA_Detour,
  ...
  if (config.file_io.capture) {
    ...
    TBF_CreateDLLHook2 ( L"kernel32.dll", "OpenFile",
                         OpenFile_Detour,
              (LPVOID *)&OpenFile_Original );

    TBF_CreateDLLHook2 ( L"kernel32.dll", "CreateFileW",
                         CreateFileW_Detour,
              (LPVOID *)&CreateFileW_Original );

    TBF_CreateDLLHook2 ( L"kernel32.dll", "ReadFile",
                         ReadFile_Detour,
