...
bool          tbf::SoundFix::wasapi_init = false;

WAVEFORMATEX tbf::SoundFix::snd_core_fmt;
WAVEFORMATEX tbf::SoundFix::snd_bink_fmt;
WAVEFORMATEX tbf::SoundFix::snd_device_fmt;

HMODULE      tbf::SoundFix::dsound_dll = 0;
HMODULE      tbf::SoundFix::ole32_dll  = 0;

WAVEFORMATEXTENSIBLE g_DeviceFormat;

const wchar_t*
TBF_DescribeHRESULT (HRESULT result)
{
  switch (result)
  {
    /* Generic (SUCCEEDED) */
                          ...
                          _Pre_null_ LPUNKNOWN      pUnkOuter)
{
  ...
  if (SUCCEEDED (ret)) {
    ...
    TBF_CreateFuncHook ( L"IDirectSound::GetSpeakerConfig",
                         vftable [8],
                         DSound_GetSpeakerConfig,
              (LPVOID *)&DSound_GetSpeakerConfig_Original );

    TBF_EnableHook (vftable [8]);

    TBF_CreateFuncHook ( L"IDirectSound::SetSpeakerConfig",
                         vftable [9],
                         DSound_SetSpeakerConfig,
              (LPVOID *)&DSound_SetSpeakerConfig_Original );

    TBF_EnableHook (vftable [9]);
    ...
    _com_error ce (ret);
    audio_log->Log ( L" > FAILURE %s - (%s)", TBF_DescribeHRESULT (ret),
                                             ce.ErrorMessage () );
  }
         ...
         )
      {
        ...
        if (pMixFormat->cbSize > sizeof (WAVEFORMATEX)) {
          if (pMixFormat->cbSize == 22) {
            memcpy (&tbf::SoundFix::snd_device_fmt, pMixFormat, sizeof (WAVEFORMATEX));
        ...
        HRESULT hr = IAudioClient_GetMixFormat_Original (This, ppDeviceFormat);
                      ...
                      _In_opt_  LPCGUID             AudioSessionGuid)
{
  ...
  if (new_session)
    memcpy (&tbf::SoundFix::snd_core_fmt, pFormat, sizeof (WAVEFORMATEX));
                    ...
                    _Out_  void        **ppInterface)
{
  ...
  if (SUCCEEDED (ret)) {
    if (iid == __uuidof (IAudioClient)) {
      ...
      TBF_CreateFuncHook ( L"IAudioClient::Initialize",
                           vftable [3],
                           IAudioClient_Initialize_Detour,
                (LPVOID *)&IAudioClient_Initialize_Original );

      TBF_EnableHook (vftable [3]);

      TBF_CreateFuncHook ( L"IAudioClient::GetMixFormat",
                           vftable [8],
                                  ...
                                  LPVOID FAR* ppv)
{
  ...
  if (SUCCEEDED (hr) && riid == __uuidof (IMMDeviceEnumerator)) {
    ...
    if (SUCCEEDED (hr2) && pDev != nullptr) {
      ...
      TBF_CreateFuncHook ( L"IMMDevice::Activate",
                           vftable [3],
...
tbf::SoundFix::Init (void)
{
  ...
  audio_log = TBF_CreateLog (L"logs/audio.log");
  ...
  TBF_EnableHook (pfnDirectSoundCreate);
  ...
  TBF_CreateDLLHook ( L"Ole32.dll", "CoCreateInstance",
  ...
  virtual SK_ICommandResult execute (const char* szArgs) {
    ...
    sprintf (info_str, "\n"
                       ...
                       "  ( Device )  SampleRate: %6lu Channels: %lu  Format: 0x%04X  BytesPerSec: %7lu  BitsPerSample: %lu\n",
                         tbf::SoundFix::snd_core_fmt.nSamplesPerSec,
                         tbf::SoundFix::snd_core_fmt.nChannels,
                         tbf::SoundFix::snd_core_fmt.wFormatTag,
                         tbf::SoundFix::snd_core_fmt.nAvgBytesPerSec,
                         tbf::SoundFix::snd_core_fmt.wBitsPerSample,

                         tbf::SoundFix::snd_device_fmt.nSamplesPerSec,
                         tbf::SoundFix::snd_device_fmt.nChannels,
                         tbf::SoundFix::snd_device_fmt.wFormatTag,
                         tbf::SoundFix::snd_device_fmt.nAvgBytesPerSec,
                         tbf::SoundFix::snd_device_fmt.wBitsPerSample);

    return SK_ICommandResult ("SoundInfo", "", info_str, 1);
  }
};

tbf::SoundFix::CommandProcessor::CommandProcessor (void)
{
  SK_ICommandProcessor& command =
    *SK_GetCommandProcessor ();

  sample_rate_   = TBF_CreateVar (SK_IVariable::Int,     (int *)&config.audio.sample_hz);
  channels_      = TBF_CreateVar (SK_IVariable::Int,     (int *)&config.audio.channels);
  enable_        = TBF_CreateVar (SK_IVariable::Boolean,        &config.audio.enable_fix);
  compatibility_ = TBF_CreateVar (SK_IVariable::Boolean,        &config.audio.compatibility);
...
bool
tbf::SoundFix::CommandProcessor::OnVarChange (SK_IVariable* var, void* val)
{
  return true;
}


tbf::SoundFix::CommandProcessor* tbf::SoundFix::CommandProcessor::pCommProc;