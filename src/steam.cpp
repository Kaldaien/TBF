...
HMODULE tbf::SteamFix::steam_dll = 0;
...
tbf::SteamFix::Init (void)
{
  ...
  TBF_CreateDLLHook ( L"steam_api.dll", "SteamVideo",
                      SteamVideo_Detour,
           (LPVOID *)&SteamVideo_Original,
           (LPVOID *)&SteamVideo );

  TBF_EnableHook (SteamVideo);
}

void
tbf::SteamFix::Shutdown (void)
{
  if (! config.steam.allow_broadcasts)
    return;

  //TBF_RemoveHook (SteamVideo);

  FreeLibrary (steam_dll);
}

void
tbf::SteamFix::SetOverlayState (bool active)
{
...
tbf::SteamFix::CommandProcessor::CommandProcessor (void)
{
  SK_ICommandProcessor& command =
    *SK_GetCommandProcessor ();

  allow_broadcasts_ = TBF_CreateVar (SK_IVariable::Boolean, &config.steam.allow_broadcasts, this);

  command.AddVariable ("AllowBroadcasts", allow_broadcasts_);
}

bool
tbf::SteamFix::CommandProcessor::OnVarChange (SK_IVariable* var, void* val)
{
...
tbf::SteamFix::CommandProcessor* tbf::SteamFix::CommandProcessor::pCommProc;